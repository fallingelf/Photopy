...
def check_images(data_out,bin=0,fits_all=None,pho_all=1):
    ...
    for i in range(len(fits_all)):
        with fits.open(fits_all[i]) as hdu:
            if 'bias' in fits_all[i].lower():
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJ_NAME'],hdu[0].header['RA'],hdu[0].header['DEC'],\
                                 hdu[0].header['EXPTIME'],hdu[0].header['FILTER'],'bias','-','-','-']  
            elif 'flat' in fits_all[i].lower():
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJ_NAME'],hdu[0].header['RA'],hdu[0].header['DEC'],\
                                 hdu[0].header['EXPTIME'],hdu[0].header['FILTER'],'flat','-','-','-']  
            else:
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJ_NAME'],hdu[0].header['RA'],hdu[0].header['DEC'],\
                                 hdu[0].header['EXPTIME'],hdu[0].header['FILTER'],'light',\
                         hdu[0].header['DATE-OBS'].replace('-',' ')+' '+hdu[0].header['TIME-OBS'].replace(':',' '),'-','-']                
...
def combine_images(images_table,trim_size,data_out):
    ...
    master_table.loc['bias']=['MasterBias.fits','NaN','Bias Frame',master_bias];
...
def aper_phot(data,header,positions,aper,data_out,gain=1,rdnoise=0):
    ...
    try:
        imageshow(data,positions,aper=aper,rim_size=32)
    except:
        return None
    return phot
...
#########################################主程序部分#############################################################################################################################
data_in='C:\\Users\\falli\\Desktop\\Project\\Photutils\\data80';os.chdir(data_in)
data_out='C:\\Users\\falli\\Desktop\\Project\\Photutils\\phres';

#----------------------------------------测光目标设置---------------------------------------------------------------------------------------
object_list=['J2359','V737Cep']                       #测光目标
ref_image_list=['J2359_I_020.fit','V737Cep_I_020.fit']            #对应的参考图
...
pixel_coo_star3=[[663,668],[878,925],[242,670],[428,868],[1028,893]]
pixel_coo_star4=[[600,650],[530,336],[1003,349],[1180,517],[240,937]]
ref_pixel_coo_list=[pixel_coo_star3,pixel_coo_star4]                       #多个目标改写为[pixel_coo_star1,pixel_coo_star2,pixel_coo_star3]
#----------------------------------------测光参数设置---------------------------------------------------------------------------------------
#Set some properties of the instrument
gain=1;
rdnoise=0;
#Photometric parameters
aper_enlarge=np.array([3,5,7])   #孔径测光参数
trim_size=np.array([[10,1280],[10,1280]]) #裁图尺寸

#----------------------------------------正式处理开始---------------------------------------------------------------------------------------
images_table=check_images(data_out,bin=0,fits_all=None,pho_all=1)    #检查相关文件flat,bias是不是齐全，并将相关文件信息存入images_table
    ...
    for k in range(len(object_list)):
        ...
        for i in range(len(image_pho_all)):
            ...
            if is_good_image(fbt_data,targe_coo):
                ...
                continue
            uct_time=middle_obs_time(fbt_header['DATE-OBS']+'T'+fbt_header['TIME-OBS'],fbt_header['EXPTIME'],'isot')
