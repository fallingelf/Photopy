...
def check_images(data_out,bin=2,fits_all=None,pho_all=1):
    images_table=pd.DataFrame(columns=('File_NAME','OBJ_NAME','RA','DEC','EXPTIME','FILTER','IMAGETYP','DATE-OBS','xbin','ybin'))
    if fits_all==None:
        fits_all=[fits_image for fits_image in os.listdir(os.getcwd()) if ('.fits' in fits_image)]    
    for i in range(len(fits_all)):
        with fits.open(fits_all[i]) as hdu:
            print(fits_all[i],hdu[0].header['OBJECT'],hdu[0].header['YFLTNM'])
            if 'bias' in hdu[0].header['OBJECT'].lower() and hdu[0].header['DETXBIN']==bin:
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJECT'].split('_')[0],'-','-','-','-','bias','-',hdu[0].header['DETXBIN'],hdu[0].header['DETYBIN']]  
            elif 'flat' in hdu[0].header['OBJECT'].lower() and hdu[0].header['DETXBIN']==bin:
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJECT'].split('_')[0],'-','-','-',hdu[0].header['YFLTNM'],'flat','-',hdu[0].header['DETXBIN'],hdu[0].header['DETYBIN']]  
            elif hdu[0].header['DETXBIN']==bin:
                images_table.loc[i]=[fits_all[i],hdu[0].header['OBJECT'].split('_')[0],split_radec(hdu[0].header['TELRA']),split_radec(hdu[0].header['TELDEC']),\
                                 hdu[0].header['EXPTIME'],hdu[0].header['YFLTNM'],'light',hdu[0].header['DATE-OBS'],hdu[0].header['DETXBIN'],hdu[0].header['DETYBIN']]                
    if (set(images_table['FILTER'][images_table['IMAGETYP']=='light']) &
        set(images_table['FILTER'][images_table['IMAGETYP']=='flat']))==\
        ...
        if not pho_all:
            0/0
        return images_table
    elif input('the flat frames is not complete, type "y" to re_check images.\n').lower()=='y':
        check_images()
    else:
        0/0            
...
def combine_images(images_table,trim_size,data_out):
    ...
    for bias_frame in images_table['File_NAME'][images_table['IMAGETYP']=='bias']:
        with fits.open(bias_frame) as hdu:
            bias_data.append(hdu[1].data[trim_size[0][0]:trim_size[0][1],trim_size[1][0]:trim_size[1][1]].astype(float))
    ...
    master_table.loc['bias']=['MasterBias.fits','NaN','bias',master_bias];
    ...
    for i in range(len(list_filter)):
        ...
        for flat_frame in images_table['File_NAME'][(images_table['IMAGETYP']=='flat') & (images_table['FILTER']==list_filter[i])]:
            with fits.open(flat_frame) as hdu:
                flat_trim=hdu[1].data[trim_size[0][0]:trim_size[0][1],trim_size[1][0]:trim_size[1][1]].astype(float)-master_bias
...
def preprocess(obj_image,images_table,master_table,trim_size,data_out):
    ...
    with fits.open(new_obj_image,mode='update') as hdu:
        hdu[1].data=(hdu[1].data[trim_size[0][0]:trim_size[0][1],trim_size[1][0]:trim_size[1][1]].astype(float)-master_table.loc['bias']['DATA'])/master_table.loc[images_table['FILTER'][images_table['File_NAME']==obj_image]]['DATA'].values[0]
        hdu[1].header.set('Trim', 'T');hdu[1].header.set('Bias',master_table.loc['bias']['File_NAME']);hdu[1].header.set('Flat',master_table.loc[images_table['FILTER'][images_table['File_NAME']==obj_image]]['File_NAME'].values[0]);
        hdu[0].header.set('FILTER',images_table['FILTER'][images_table['File_NAME']==obj_image].values[0])
        hdu[0].header.set('GAIN',hdu[1].header['GAIN']);hdu[0].header.set('RDNOISE',hdu[1].header['RDNOISE'])
        hdu.flush();
    return new_obj_image,hdu[0].header,hdu[1].data
...
def image_shift(fbt_data,ref_data,percent_fit=0.6,precision=100):
...
def pro_photo(object_list,ref_image_list,ref_pixel_coo_list,aper_enlarge,trim_size,data_in,data_out,pho_all=1):
    images_table=check_images(data_out,bin=2,pho_all=pho_all)       #检查相关文件flat,bias是不是齐全，并将相关文件信息存入images_table
    ...
    if len(object_list)==len(ref_image_list) and len(object_list)==len(ref_pixel_coo_list):
        ...
        for k in range(len(object_list)):
            ...
            if -1 in ref_fwmh:
                print('the reference image is terriblly bad!',0/0)
            aper=np.median(ref_fwmh)*aper_enlarge;print(object_name,':  Photometrical aperture= ',aper[0])   
            ...
            for i in range(len(image_pho_all)):
                ...
                if is_good_image(fbt_data,targe_coo):
                    ...
                    continue
                uct_time=middle_obs_time(fbt_header['DATE-OBS'],fbt_header['EXPTIME'],'isot')
                if len(phot_table)==len(ref_coo):
                    for j in range(len(phot_table)):
                        pho_res.loc[str(i)+'_'+str(j)]=[uct_time.value,\
                                round(phot_table['magnitude'][j],3),fbt_header['EXPTIME'],fbt_header['FILTER'],\
                                image_pho,uct_time.jd,round(phot_table['delta_magnitude'][j],3),str(j+1),\
            ...
            ra=images_table['RA'][images_table['File_NAME']==ref_image_list[k]].values[0].replace(':',' ');
            dec=images_table['DEC'][images_table['File_NAME']==ref_image_list[k]].values[0].replace(':',' ')
...
##########################################主程序部分#############################################################################################################################
data_in='C:\\Users\\falli\\Desktop\\Project\\Photutils\\data240';os.chdir(data_in)
data_out='C:\\Users\\falli\\Desktop\\Project\\Photutils\\phres';
#----------------------------------------测光目标设置---------------------------------------------------------------------------------------
object_list=['J162117']                       #测光目标
ref_image_list=['YFCd180428.fits']            #对应的参考图
pixel_coo_star1=[[545,605],[680,642],[781,491],[445,799],[276,305]] #对应的参考图上的目标位置,多个则分别命名为pixel_coo_star2,pixel_coo_star3....
#pixel_coo_star2=[[523,592],[548,510],[317,605],[477,732],[690,488]]
ref_pixel_coo_list=[pixel_coo_star1]                       #多个目标改写为[pixel_coo_star1,pixel_coo_star2,pixel_coo_star3]
#----------------------------------------测光参数设置---------------------------------------------------------------------------------------
aper_enlarge=np.array([1,3,4])   #孔径测光参数
trim_size=np.array([[200,850],[200,850]]) #裁图尺寸[x_min:x_max,y_min:y_max]


if __name__ == "__main__":
    pro_photo(object_list,ref_image_list,ref_pixel_coo_list,aper_enlarge,trim_size,data_in,data_out,pho_all=1);
